 ...
 * @returns {Promise<any>}
 */
const getFormData = async (sessionId) => {
 ...
 * @returns {Promise<any>}
 */
const getValidationErrors = async (sessionId) => {
  const storedData = await getSessionData(sessionId);
 ...
 * @returns {Promise<*>}
 */
const getDataForPage = async (sessionId, pageId) => {
 ...
 */
const setFormData = async (sessionId, data) => {
  try {
 ...
 * @returns {Promise<void>}
 */
const setValidationErrors = async (sessionId, validation) => {
  try {
    const storedData = await getSessionData(sessionId) || Object.create(null);
    if (process.env.SESSION_STORAGE === 'redis') {
      const updatedData = Object.assign(storedData, {
 ...
 * @returns {Promise<void>}
 */
const setDataForPage = async (sessionId, pageId, pageData) => {
  try {
...
const setValidationErrorsForPage = async (sessionId, pageId, validationData) => {
  try {
...
const getValidationErrorsForPage = async (sessionId, pageId) => {
  const validationErrors = await getValidationErrors(sessionId)
  return validationErrors?.[pageId];
}

const clearValidationErrorsForPage = async (sessionId, pageId) => {
  try {
    const storedValidationData = await getValidationErrors(sessionId) || Object.create(null);
