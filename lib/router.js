...
export default async function router(rootPage, sessionId, basePath = '') {
  console.log('Router running ...');

  if (rootPage) {
    console.log(`Root page is '${rootPage}'`);
    return await handleRouting(rootPage, basePath, sessionId);
  } else {
    console.log('No root page set, aborting. Routing complete');
  }
}

async function handleRouting(pageId, basePath, sessionId) {
  console.log(`Checking config for page '${pageId}'`);
  let config;
  try {
    config = await import(`../page-configurations${basePath}/${pageId}`);
  } catch (err) {
  ...
  if (!nextPageData) {
    ...
    return await handleRouting(nextPage, basePath, sessionId);
  }
}
